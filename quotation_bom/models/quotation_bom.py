...
class QuotationBom(models.Model):
    ...
    state = fields.Selection([('quotation','QUOTATION'),('sale','SALE')],string="State",
        readonly=True,
        store=True,
        copy=False,
        index=True,
    ...
    def get_bom_details(self):
        ...
        for i in self.qbom_lines:
            if i.level == '0':
                i.product_upload_file = i.product_id.product_tmpl_id.product_file
                i.filename = i.product_id.product_tmpl_id.filename
    ...
    def set_bom(self):
        ...
        if self.product_id:
            ...
            bom_objs_tmpl=self.env['mrp.bom'].search([('product_tmpl_id','=',self.product_id.product_tmpl_id.id)])
            if bom_objs:
                for bom in bom_objs:
                    il.append(bom.id)
    ...
    def create(self, vals):
        ...
        if vals:
            ...
            #     "quotation.bom")
            vals['name'] += '-'+ sol_id.product_id.name
...
class quotation_bom_line(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            if rec.product_id.name:
                ...
                name = ""
            if rec.qbom_id:
                bom_name = rec.qbom_id.order_line_id.order_id.name
            else:
                bom_name = ""
            if rec.product_id:
                rec_name = rec.product_id.name
            else:
                rec_name = ""
            result.append((rec.id,bom_name if bom_name else '' + ' ' +name if name else ''))
    ...
    #Created by Ajinkya on  16/09/2019
    computed_budget = fields.Float("Computed Budget")
    budget = fields.Float("Budget")
    state=fields.Selection(related="qbom_id.state",readonly=True,store=True)
    
    #Created by Ajinkya on 11/10/2019
    # product_document = fields.Binary(string="Product Document", attachment="True")
    filename = fields.Char("Filename")
    product_upload_file=fields.Binary(string="Product Document")
    upload_pdf_file =fields.Binary(string="Upload Files")
    fname = fields.Char("Filename")
    # route_ids=fields.Many2many(related="product_id.route_ids",readonly=True,store=True)
    @api.depends('level')
    ...
    #Created By Ajinkya on 11/10/2019
    @api.onchange('upload_pdf_file')
    def pdf_files_only(self):
        print("FIOLEEEEEEEEEEEEEEEEEEEEE",self.fname)
        file=self.fname
        if not file.endswith('.pdf'):
            raise ValidationError(_('Please Upload Pdf File'))
    ...
    def set_bom_parent_id(self):
        ...
        if 'default_qbom_id' in self.env.context:
            if 'order_id' in self.env.context:
                ...
                qbom_obj=self.env['quotation.bom.line'].browse(order_id)
                level= self.level
                qbom_lines=self.env['quotation.bom.line'].search([('id','child_of',qbom_obj.id),('level','=',str(int(level)-1))])
            # for order in self:
                # if order.qbom_lines:
                #     for i in order.qbom_lines:
                for i in qbom_lines:
    ...
    def create(self, vals):
        ...
        for order in new_id:
            ...
            if order.level != '0':
                if not order.bom_parent_id:
                    raise UserError(_("Please Enter the Quote Parent Item"))    
        return new_id
    ...
    def onchng_set_bom(self):
        ...
        if self.product_id and self.product_tmpl_id:
            ...
            self.product_document = self.product_id.product_doc 
    ...
    def _onchange_set_defaultcode(self):
        for order in self:
            if order.product_id:
                ...
                order.name = order.product_id.name
    ...
    def write(self,vals):
        ...
        if vals:
            if 'budget' in vals:
                if self.bom_parent_id:
                    self.bom_parent_id.budget+=vals['budget']
    ...
    def action_child_items(self):
        tree = self.env['ir.model.data'].xmlid_to_res_id(
                    'quotation_bom.quotation_bom_line_new_tree123', raise_if_not_found=True) 
        for order in self:
            li=[]
            if order.child_id:
                if 'l_list' in self.env.context:
                    print("self.env.context['l_list']",self.env.context['l_list'])
                    li+=self.env.context['l_list']
                else:
                    other_parents=self.search([('qbom_id','=',order.qbom_id.id),('level','=',0)])
                    if other_parents:
                        for x in other_parents:
                            li.append(x.id)
                for j in order.child_id:
                    li.append(j.id)
                l_list=[]
                li=list(set(li))
                result= {
...
#Roll up amount of child budget to parent 
    #Created by Ajinkya on  16/09/2019
    # @api.depends('child_id.budget')
    # def set_budget_new(self):
    #     # computed_budget=0.0
    #     for order in self:
    #         if order.child_id:
    #             for child in order.child_id:
    #                 order.computed_budget += child.budget
    #             # order.computed_budget = computed_budget
    #             print("Computed Budget ## 11111",order.computed_budget)
    #         else:
    #             order.computed_budget=order.budget
    #             print("Computed Budget ## 22222",order.computed_budget)
    # @api.onchange('budget')
    # def set_computed_budget(self):
    #     self.computed_budget=self.budget
    #Created by Ajinkya on  16/09/2019                                                                                     
    # @api.depends('child_id.budget')
    ...
    #         if order.child_id:
    #             for child in order.child_id:
    #                 order.budget+=child.budget
    #                 print("Budget======",order.budget)
    #         else:
    #             order.budget=order.budget
    # @api.depends('computed_budget')
    # def _set_budget(self):
    #     for order in self:
    #         if order.child_id:
    #             #if order.computed_budget:
    #             order.budget=order.computed_budget
    #         else:
    #             order.budget=order.budget
    #             print("budget # 44444",order.budget)